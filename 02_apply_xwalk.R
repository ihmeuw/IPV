...
# 
# Author: FILEPATH
# Purpose: Apply crosswalks to alternate definition SDG data
#
####################################################################################################################################################################
...
#consider intercourse only definitions as sexual IPV definitions
data[var=='int_sexpv_pastyr', var:='any_sexpv_pastyr']
data[var=='intsex_anyphys_ipv_pastyr', var:='anysex_anyphys_ipv_pastyr']

#set variables of 2019 cv-tagged data
data[cv_sdg_ipv_no_sexual==1, var:='any_physpv_pastyr']
data[cv_sdg_ipv_no_physical==1, var:='any_sexpv_pastyr']

#split into gold-standard and alternate data sets
data_unadj <- data[var %in% c('anysex_anyphys_ipv_pastyr')]
data_to_adjust <- data[!var %in% c('anysex_anyphys_ipv_pastyr')]
...
compare <- as.data.table(read.xlsx('FILEPATH'))
compare[!nid %in% unique(data_to_save$nid)]

#save file
write.xlsx(data_to_save, 'FILEPATH', sheetName='extraction')
table(data_to_save$var)
