...
#
# Author: AUTHOR
# Purpose: Rake level 5 and 6 subnats to raked level 4 parent subnats using LBD logit-raking function; levels 5 & 6
#
#####################################################################################################################################################################################################################################
rm(list=ls())
...
#Arguments
if (interactive()) {
  version <- 'regional_data_range_phi0.9'
  run_id <- 179951
  l <- 25318
} else {
...
#read in level 5 children for this parent id
file.list <- paste0(root, "forecasted_draws/", run_id, '/', version, '/', c(as.character(unique(locs[parent_id==l]$location_id))), '.csv')
...
lvl4 <- fread(paste0(root, "raked_draws/", run_id, '/', version, '/', l, '.csv'))
lvl4 <- lvl4[, .(value = mean(raked_value)), by=.(location_id, age_group_id, year_id)] #just the mean, not draw level
...
dir.create(paste0(root,"raked_draws/",run_id,"/", version, "/"), recursive=T)
for(c.loc in unique(raked$location_id)){
  print(c.loc)
  write.csv(raked[location_id == c.loc], paste0(root,"raked_draws/",run_id,"/", version, "/", c.loc,".csv") )
}
